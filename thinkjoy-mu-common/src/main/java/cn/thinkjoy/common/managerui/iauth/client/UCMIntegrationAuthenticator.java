 ...
 */
@Component
public class UCMIntegrationAuthenticator extends Authenticator implements HttpRquestConstant {

    private static Logger logger = LoggerFactory.getLogger(UCMIntegrationAuthenticator.class);
    ...
    private UCMAuthRequestFactory authRequestFactory = new UCMAuthRequestFactory();

    public UCMIntegrationAuthenticator() {
        // for spring bean initial
    }

    public UCMIntegrationAuthenticator(List<TokenHandler> tokenHandlers) {
        init(tokenHandlers);
    }
...
//        params.put() FIXME
        ...
        UCMAuthRequest request = (UCMAuthRequest) baseRequest;
    ...
    public void authenticationDone(BaseRequest baseRequest) {
        //TODO
        Principal<User> principal = ((UCMAuthRequest) baseRequest).getPrincipal();
...
//            baseRequest.getRequest().setAttribute(HTTP_ATTRIBUTE_USER_DETAIL, principal.getOwner());
    ...
    public void embedmentDone(BaseRequest baseRequest) {
        Principal principal = ((UCMAuthRequest) baseRequest).getPrincipal();
